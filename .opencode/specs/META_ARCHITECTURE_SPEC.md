...
- **开发向内求解 (Inward Solving First)**：解决问题时的出发点和首选方向是向内求解。当系统报错时，Agent 必须首先审视自身（源码、基建结构、业务逻辑）是否存在缺陷，通过改进智能体自身来解决问题。只有在确定智能体本身代码逻辑毫无缺陷后，才允许转向外部，排查外部运行环境是否异常。严禁一开始就通过随意修改外部环境（如乱改宿主机配置）来掩盖残缺的代码。
- **应用向外求解 (Outward Solving First)**：解决问题时的出发点和首选方向是向外求解。当无法满足用户需求或报错时，Agent 必须首先排查外部环境、用户参数是否异常。只有在确定外部环境完全无误后，才转向内部，判定为智能体本身存在缺陷。此时，严禁应用模式自行修改源码，必须直接触发反哺机制（写入 REQUESTS.md），将“修改智能体自身”的任务交由对应的开发角色处理。
## 二、 三态 Agent 角色定义
