...
- **应用模式（App）必须“求解向外优先”**：解决问题时的首选原生方案是向外求解。Agent 必须首先尝试调整外部环境、用户输入来解决特定场景。如果确定外部环境没问题，或者改变外部环境的方式是非原生的（Hack/打补丁），则转向内部求解。此时，严禁应用模式自行修改源码，必须直接触发**反哺机制**（写入 REQUESTS.md），将“向内修改智能体”的任务交由开发角色处理。
## 二、 三态 Agent 角色定义
