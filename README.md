...
n👉 **想了解 Onebot 的诞生故事与架构推导过程？请阅读 [Onebot 起源与架构演进史](.opencode/specs/ORIGIN.md)**
...
- **App -> Biz Dev**: 用户在 App 模式反馈问题时，App Agent 无权修改代码，会自动将工单写入业务需求池（项目根目录 `REQUESTS.md`），并提示用户切换至 `biz-dev` 处理。
- **Biz Dev -> Platform Dev**: 研发遇到平台规则死板或通用工具 Bug 时，无权修改基建，会自动将建议写入平台基建池（`.opencode/REQUESTS.md`），并提示用户切换至 `platform-dev` 升级底座。
